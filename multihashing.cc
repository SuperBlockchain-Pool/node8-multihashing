...
#define DECLARE_INIT(x) \
    void x(Local<Object> exports, Local<Value> module, Local<Context> context)
...
DECLARE_FUNC(scrypt) {
    ...
    unsigned int nValue = args[1]->Uint32Value(context).FromJust();
    unsigned int rValue = args[2]->Uint32Value(context).FromJust();
...
DECLARE_FUNC(scryptn) {
    ...
    unsigned int nFactor = args[1]->Uint32Value(context).FromJust();
...
DECLARE_FUNC(scryptjane) {
    ...
    int timestamp = args[1]->Int32Value(context).FromJust();
    int nChainStartTime = args[2]->Int32Value(context).FromJust();
    int nMin = args[3]->Int32Value(context).FromJust();
    int nMax = args[4]->Int32Value(context).FromJust();
...
DECLARE_FUNC(cryptonight) {
    ...
    if (args.Length() >= 2) {
        if(args[1]->IsBoolean())
            ...
            cn_variant = args[1]->Uint32Value(context).FromJust();
        else
...
DECLARE_FUNC(cryptonightsoftshell) {
    ...
    if (args.Length() >= 3) {
      if (args[2]->IsUint32())
        height = args[2]->Uint32Value(context).FromJust();
      else
    ...
    if (args.Length() >= 4) {
      if (args[3]->IsUint32()) {
        CN_SOFT_SHELL_MEMORY = args[3]->Uint32Value(context).FromJust();
    ...
    if (args.Length() >= 5) {
      if (args[4]->IsUint32())
        CN_SOFT_SHELL_WINDOW = args[4]->Uint32Value(context).FromJust();
      else
    ...
    if (args.Length() >= 6) {
      if (args[5]->IsUint32())
        CN_SOFT_SHELL_MULTIPLIER = args[5]->Uint32Value(context).FromJust();
      else
